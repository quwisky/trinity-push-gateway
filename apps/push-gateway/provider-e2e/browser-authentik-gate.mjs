...
if (!statePath) {
  throw new Error('Browser state path is required.');
}
const state = JSON.parse(await readFile(statePath, 'utf8'));
if (state.provider !== 'authentik') {
  throw new Error('The legacy browser gate supports only Authentik.');
}
...
async function authenticateProvider(page, identity) {
...
async function login(browser, identity, allowed) {
  ...
  try {
    ...
    await authenticateProvider(page, identity);
...
try {
  ...
  console.info(
    'Authentik assembled-image login, group rejection, local logout, and no-external-request contracts passed.',
  );
} finally {
  await browser.close();
}
