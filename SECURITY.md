...
The optional self-hosted administration surface is disabled by default and is
never present in the Cloudflare Worker. Operators must use an exact HTTPS public
origin, a confidential PKCE-capable OIDC client, exact group authorization,
Docker secret files, and proxy-owned HSTS/compression. Opaque local sessions use
Secure, HttpOnly, SameSite cookies and exact-origin session-bound CSRF checks;
no provider token or raw claim is persisted. `admin.sqlite`, audit data, session
state, and backups are sensitive even though notification content and raw
Matrix/FCM identifiers are excluded. See the
[administration guide](docs/deployment/self-hosting/administration.md).

Administration assets, authentication, storage, metrics, and operator actions
are fail-closed and isolated from public health and Matrix delivery. Recovery
must never alias `admin.sqlite` to `gateway.sqlite`. A full-volume backup is
required to preserve both databases; a UI-created backup contains delivery
state only.
