...
The standalone repository is `https://github.com/quwisky/trinity-push-gateway`. Its default branch is `master`. Nx 23 orchestrates a root-managed pnpm workspace: `apps/gateway` is the single implemented `push-gateway` project, while `apps/push-gateway-ui` is documentation-only until a dedicated Angular task begins. Worker and Bun remain adapters within the same project because they implement and release one Push Gateway contract. Nx Core uses explicit targets and a local cache for deterministic validation and filesystem outputs; Nx Cloud, Nx Release, speculative internal libraries, and affected-only CI are deferred.
