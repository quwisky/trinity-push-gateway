...
It accepts notification requests from arbitrary Matrix homeservers. It does not own users, client installation registration, Matrix sync, event retrieval, an implemented administration UI, or third-party Firebase credentials. The workspace reserves `push-gateway-ui` as a future self-hosted administration application, but its API, authentication, and implementation are not yet defined.
...
The standalone repository is `https://github.com/quwisky/trinity-push-gateway`. Its default branch is `master`. Nx 23 orchestrates a root-managed pnpm workspace: `apps/push-gateway` contains the implemented runtime, `docs/` is the publishable `push-gateway-docs` project, and `apps/push-gateway-ui` remains documentation-only until a dedicated Angular task begins. Worker and Bun remain adapters within one project because they implement and release one Push Gateway contract. Official Nx plugins infer project-scoped ESLint and run-mode Vitest targets; Nx Core keeps explicit targets for Wrangler, Bun, VitePress, TypeScript, migration, coverage, bundle-policy, and aggregate work. Deterministic validation and filesystem outputs use the local cache, while affected-only CI preserves required check names and skips unrelated gateway and container work. Nx Cloud, Nx Release, speculative internal libraries, and premature Angular integration remain deferred.
