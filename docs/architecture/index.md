...
The Push Gateway UI is disabled by default and available only with the Bun target. When enabled, one origin serves the Angular application at `/admin/`, the versioned operator API at `/admin/api/v1/`, and fixed OIDC routes at `/admin/auth/`; canonical OpenAPI 3.1.2 defines the browser API, and CORS is not supported. Operator Session, Operator Session list, safe configuration, Overview, and Metrics responses are authored as runtime-neutral Zod contracts. The Metrics contract also owns accepted intervals, paired UTC range input, its 24-hour default, and its 30-day maximum; Overview operation summaries expose only a finite set of privacy-safe reason codes. Bun validates and normalizes through those canonical definitions, deterministic generation materializes their marked OpenAPI components, query parameters, and browser policy, and Orval derives the Angular client from the published contract. Former Bun schemas remain only as migration compatibility validators in parity fixtures until the remaining operator contracts migrate. A Bun-only `openid-client` module performs generic standards-based OIDC while gateway-owned policy establishes time-bounded Operator Sessions for Operator Identities, and the browser receives only an opaque host-only session cookie.
...
The implementation uses one runtime-neutral request core with narrow storage and limiter ports. Cloudflare composes the module Worker, D1, rate-limit binding, and cron adapters; the self-hosted target composes Bun 1.4, built-in SQLite, an in-memory limiter, internal cleanup scheduling, a Bun-only Hono administration router, and a Bun-only `openid-client` protocol module for the optional operator surface. The delivery routers use direct `fetch`; Hono is confined to `/admin/*`. Both runtimes use native Web Crypto, `jose` for service-account JWT signing, and Zod for untrusted-boundary validation. The administration-contract module owns response semantics and their OpenAPI projection behind one interface; Hono and the generated Angular client consume those artifacts rather than restating constraints. The runtime-specific D1 and Bun storage adapters use Drizzle for typed queries while retaining reviewed SQL for exact transaction, migration, and runtime behavior. Runtime dependencies are exactly pinned and allowlisted; Better Auth, Firebase Admin, Google Auth, retry libraries, general cross-runtime database abstractions, and external logging SDKs remain excluded. Development uses pnpm 11, Node 24, strict TypeScript, Vitest, Bun's test runner for runtime integration, ESLint, Prettier, Wrangler, Commitlint, Husky, lint-staged, Dependabot, Docker, Playwright, and GitHub Actions with immutable action pins.

Cloudflare resources and configuration are managed through the app-owned Wrangler configuration. All external runtime settings and secrets share the `TRINITY_PUSH_GATEWAY_` namespace across Worker and Bun, and Cloudflare exposes the `TRINITY_PUSH_GATEWAY_DB` and `TRINITY_PUSH_GATEWAY_SOURCE_RATE_LIMITER` bindings. One deep configuration-catalog module owns every setting's default, constraint, secrecy, runtime applicability, validation, safe operator projection, and public reference metadata. Its focused public interface provides lookup and runtime-filtered listing; name maps, default maps, parsing schemas, and safe-projection rules remain internal rather than being re-exported as compatibility ledgers. Narrow Worker and Bun adapters supply runtime capabilities such as secret-file reads, trusted-proxy validation, and hashing without pulling the Bun administration graph into Cloudflare. Deployment contracts resolve the Compose model and exercise the effective Bun and administration configuration instead of scanning configuration source text, while generated documentation and the read-only Configuration page consume the same catalog knowledge. Self-hosting uses a hardened non-root, read-only container with a local `/data` volume, Docker secret files, external TLS termination, explicit proxy trust, startup migrations, graceful shutdown, health probing, and a verified backup command. Firebase project setup, platform registration, APNs credentials, and least-privilege service-account creation remain documented operator steps. Secrets are injected at runtime and never stored in Git.
...
Automated coverage includes validation, payload mapping, OAuth signing, FCM error classification, redaction, shared D1/SQLite storage behavior, Matrix contract fixtures, multi-account behavior, mixed outcomes, concurrent duplicates, proxy address handling, limiter bounds, migrations, persistence, backup integrity, Bun HTTP lifecycle, Cloudflare Free-plan bundle limits, container contents, size, identity, read-only operation, administration isolation, the exact runtime dependency allowlist, and executable Nx project targets. FCM remains mocked; administration authentication is exercised through a deterministic protocol suite and disposable real Pocket ID and Authentik services in container CI.
