...
- **PASS**: tagged source and/or a committed contract test proves the behavior.
...
| --------------------------------------------------- | ------------------------------------------------------- | -------------------------------- | -------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Generic Pocket ID and Authentik OIDC                | **PASS by protocol inspection; candidate not retained** | N/A                              | **PASS for deterministic provider profiles; live-product smoke deferred to #24** | Bun drives the same confidential code flow against Pocket-ID-like `groups` scope semantics and Authentik-like `profile`-mapped groups. The product-specific scopes and issuer constraints come from the primary provider sources above.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| One-use, five-minute login state                    | **FAIL — first disqualifier**                           | **FAIL**                         | **PASS** by gateway ownership                                                    | Better Auth stores ten-minute state and performs separate read/delete operations ([state creation](https://github.com/better-auth/better-auth/blob/v1.7.2/packages/better-auth/src/oauth2/state.ts#L56-L74), [state parsing](https://github.com/better-auth/better-auth/blob/v1.7.2/packages/better-auth/src/state.ts#L215-L290)). With `openid-client`, the gateway can atomically `DELETE … RETURNING` a row whose expiry is at most five minutes before exchanging the code.                                                                                                                                                                                                                                         |
...
| Immediate revocation                                    | **PASS** with cookie cache off               | **PASS**                               | **PASS** by gateway ownership                                                      | Better Auth has database-backed revoke endpoints and cache controls ([session API](https://github.com/better-auth/better-auth/blob/v1.7.2/packages/better-auth/src/api/routes/session.ts)); a gateway-owned session lookup and delete gives the exact required semantics without another endpoint surface.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| Bun bundle and image budget                             | **FAIL** relative to the focused alternative | **FAIL**                               | **PASS for the selected module and existing image policy**                         | The comparative proxy measured Better Auth at 607,493 raw/165,968 gzip bytes versus `openid-client` at 28,131 raw/9,253 gzip bytes. The retained, validated module is 53,722 raw/17,206 gzip bytes; the existing Bun entry remains 269,812 raw bytes until #24 wires it. Container CI retains the 150 MiB compressed policy.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
- `bun test test/bun/auth` passes 36 tests. A Bun 1.4 client drives an exact
...
- A temporary Bun/Drizzle SQLite feasibility adapter persists only the typed
  ...
  keying, profile pruning, and five/100 session caps also pass. The production
  `/data/admin.sqlite` schema and the same contract remain #24 work.
...
- `pnpm nx run push-gateway:check-auth-selection` bundles the retained module at
  ...
  rejects every Better Auth package.
- `pnpm nx run push-gateway:check-bundle` reports the unchanged Worker at
  ...
  gzip bytes. The last green AMD64 image baseline is 163,202,149 unpacked and
  73,776,077 compressed bytes; pull-request container CI supplies the final
  comparison against the retained 157,286,400-byte compressed ceiling.
...
- 30-minute idle and eight-hour absolute boundaries with a fake clock; policy
  fingerprint invalidation; immediate logout/revocation; deterministic oldest
  eviction at five per identity and 100 globally, including concurrent logins.
...
- Guaranteed local revocation plus best-effort provider logout through a
  discovered endpoint without retaining a provider token; no portable
  post-logout return is claimed.
- Fault injection after identity and session writes: the complete feasibility
  transaction either commits or rolls back.
- Provider or administration-database outage cannot alter Matrix notification
  delivery or health behavior.
- Bun build/import smoke, real compressed application and image budgets, and a
  Worker source-map assertion showing no `openid-client`, `oauth4webapi`, or
  Bun administration module in the Worker graph and no unexplained Worker
  bundle delta. Existing Worker `jose` inputs remain valid for FCM signing.
